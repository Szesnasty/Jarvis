...
class EnrichmentPayload(BaseModel):
    ...
    actionable_next_step: str = Field(min_length=1, max_length=200)
    work_type: str
    business_area: str
    execution_type: str
    risk_level: str
    ambiguity_level: str
    hidden_concerns: list[str] = Field(default_factory=list, max_length=5)
    likely_related_issue_keys: list[str] = Field(default_factory=list, max_length=10)
    likely_related_note_paths: list[str] = Field(default_factory=list, max_length=10)
    ...
    def _validate_keywords(cls, value: list[str]) -> list[str]:
        ...
        if len(clean) < 3:
            raise ValueError("keywords must have 3-8 values")
        return clean[:8]
